...
- **Monitoring** (`monitoring.js`): Sentry wrapper that is a **no-op unless a DSN
  is configured** (`expo.extra.sentryDsn` / `SENTRY_DSN`). No PII, errors only.
...
- **Build-time flags:** `EXPO_PUBLIC_E2E` (test fixtures), `SENTRY_DSN` (crash
  reporting). `EXPO_PUBLIC_*` vars are inlined by Metro at bundle time.
...
| `src/monitoring.js` | Sentry wrapper (off without DSN) |
